            ...
            for im in os.listdir(folder + d_set + '/imgs'):
                ...
                for i in range(0, img.shape[0], overlap_size):
                    for j in range(0, img.shape[1], overlap_size):
                        ...
                        if i + patches_size <= img.shape[
                                0] and j + patches_size <= img.shape[1]:
                            p = img[i:i + patches_size, j:j + patches_size, :]
                            gt_p = gt_img[i:i + patches_size,
                                          j:j + patches_size, :]

                        elif i + patches_size > img.shape[
                                0] and j + patches_size <= img.shape[1]:
                            p = (np.ones((patches_size, patches_size, 3)) -
                                 random.randint(0, 1)) * 255
                            gt_p = np.ones(
                                (patches_size, patches_size, 3)) * 255
                            ...
                            p[:,
                              0:img.shape[1] - j, :] = img[i:i + patches_size,
                                                           j:img.shape[1], :]
                            gt_p[:, 0:img.shape[1] -
                                 j, :] = gt_img[i:i + patches_size,
                                                j:img.shape[1], :]
                        ...
                        cv2.imwrite(main_path + 'train/' + str(n_i) + '.png',
                                    p)
                        cv2.imwrite(
                            main_path + 'train_gt/' + str(n_i) + '.png', gt_p)
                        n_i += 1
        if d_set == test_set:
            for im in os.listdir(folder + d_set + '/imgs'):
                img = cv2.imread(folder + d_set + '/imgs/' + im)
            ...
            for im in os.listdir(folder + d_set + '/imgs'):
                ...
                for i in range(0, img.shape[0], patches_size_valid):
                    for j in range(0, img.shape[1], patches_size_valid):
                        ...
                        cv2.imwrite(
                            main_path + 'valid/' + im.split('.')[0] + '_' +
                            str(i) + '_' + str(j) + '.png', p)
                        cv2.imwrite(
                            main_path + 'valid_gt/' + im.split('.')[0] + '_' +
                            str(i) + '_' + str(j) + '.png', gt_p)
